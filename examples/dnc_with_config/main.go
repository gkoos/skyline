...
func main() {
	...
	// Set DNCConfig for custom threshold and batch size
	// These values control the recursion and batching in the DNC algorithm
	skyline.DNCConfig.Threshold = 10
	skyline.DNCConfig.BatchSize = 10
	...
	result, err := skyline.Skyline(data, dims, prefs, "dnc")
	if err != nil {
		panic(err)
	}
	fmt.Println("Skyline (DNC, custom config):", result)
