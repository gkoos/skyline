...
func TestDNC_Skyline_Large(t *testing.T) {
	tests := []struct {
		name     string
		input    types.Dataset
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := DivideAndConquer(tc.input, tc.prefs, cfg)
			if !equalSkylineSet(result, tc.expected) {
				t.Errorf("DNC skyline incorrect for %s: got %v, want %v", tc.name, result, tc.expected)
			}
		})
	}
}
